...
- **Модерация** — спорные скидки админу в ЛС (кнопки «Принять» / «Отклонить»)
...
| Площадка | Списки категорий | Карточки | Причина ограничений |
|----------|------------------|----------|---------------------|
| **Яндекс Маркет** | ✅ работает | ✅ ~95% | Основной источник данных сейчас |
| **Wildberries** | ❌ 0 товаров | — | API категорий возвращает пустой список |
| **Ozon** | ❌ | — | `403 Forbidden` с IP Docker / дата-центра |

Типичный обход: **~68 товаров** (только ЯМ, 8 категорий × ~8–9 шт., лимит до 20 на МП).

**Почему скидок может быть 0:** у ЯМ часто нет «старой цены» в JSON-LD → скидка парсера `null` → фильтр 15% отсекает. Скидка по истории БД появляется после нескольких дней обходов.

**Что помогает:** прокси/модем (`PROXY_LIST`), починка краулера WB, улучшение парсера ЯМ, `DATA_COLLECTION_WARMUP_DAYS=3` на период накопления истории.
